 ...
 */
const generateReportPDF = (report, details, empresaNombre, empresaSlug, outputPath) => {
    return new Promise((resolve, reject) => {
        try {
            ...
            doc.fillColor('#64748b') // Slate 500
               .fontSize(8)
               .text('SISTEMA INTEGRADO DE CONTROL Y CÁMARAS', 50, 53, { characterSpacing: 1.2 });
            ...
            doc.text(`Centro Comercial: `, 65, detailsY + 9, { bold: true, continued: true })
            ...
            doc.text(`Fecha de Reporte: `, 320, detailsY + 16, { bold: true, continued: true })
            ...
            // Columna 1: Total
            doc.fillColor(SECONDARY_COLOR).fontSize(8).text('TOTAL CÁMARAS', 50, cardY + 12, { align: 'center', width: 166 });
            doc.fillColor(PRIMARY_COLOR).fontSize(14).text(`${report.total_camaras}`, 50, cardY + 24, { align: 'center', width: 166, bold: true });

            // Columna 2: Operativas
            doc.fillColor(SUCCESS_COLOR).fontSize(8).text('OPERATIVAS', 216, cardY + 12, { align: 'center', width: 167 });
            doc.fillColor(SUCCESS_COLOR).fontSize(14).text(`${report.operativas}`, 216, cardY + 24, { align: 'center', width: 167, bold: true });

            // Columna 3: No Operativas
            doc.fillColor(DANGER_COLOR).fontSize(8).text('CON FALLA', 383, cardY + 12, { align: 'center', width: 167 });
            doc.fillColor(DANGER_COLOR).fontSize(14).text(`${report.no_operativas}`, 383, cardY + 24, { align: 'center', width: 167, bold: true });
            ...
            for (const propName of Object.keys(groupedDetails)) {
                ...
                doc.fillColor(PRIMARY_COLOR).fontSize(7.5).text(`PROPIETARIO: ${propName.toUpperCase()}`, 60, y + 4.5, { bold: true });
                y += 16;

                // Renderizar cámaras de este propietario
                for (const item of items) {
                    const nameHeight = doc.fontSize(8).heightOfString(item.nombre_camara, { width: 170 });
                    let subText = [];
                    ...
                    if (subText.length > 0) {
                        subTextHeight = doc.fontSize(7).heightOfString(subTextStr, { width: 170 });
                    }
                    ...
                    if (y + rowHeight > 675) {
                        ...
                        doc.rect(50, y, 500, 16).fill('#e2e8f0');
                        doc.fillColor(PRIMARY_COLOR).fontSize(7.5).text(`PROPIETARIO: ${propName.toUpperCase()} (CONTINUACIÓN)`, 60, y + 4.5, { bold: true });
                        y += 16;
                    }
                    ...
                    doc.fillColor(TEXT_COLOR).fontSize(8);
                    doc.text(`${item.sector_nombre}\n${item.nivel_nombre}`, 240, y + 4, { width: 110 });
                    doc.text(`${item.tipo_nombre}\n${item.modelo_nombre}`, 360, y + 4, { width: 110 });

                    const dotY = y + (rowHeight / 2);
                    doc.fillColor(item.estado ? SUCCESS_COLOR : DANGER_COLOR).circle(488, dotY, 3.5).fill();
                    doc.fillColor(TEXT_COLOR).text(item.estado ? 'Operativa' : 'Falla', 496, dotY - 4, { width: 50 });
            ...
            for (let i = range.start; i < range.start + range.count; i++) {
                ...
                doc.fontSize(7)
                   ...
                   .text('SECAM - Sistema de Control e Inspección de Cámaras de Seguridad', 50, 748, { align: 'left', lineBreak: false });
                   
                doc.text(`Página ${i + 1} de ${range.count}`, 450, 748, { align: 'right', width: 100, lineBreak: false });
